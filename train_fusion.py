...
import test_fuse  # import test_fuse.py to get mAP after each epoch
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                results, maps, times = test_fuse.test(data_dict,
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = test_fuse.test(opt.data,
                                          batch_size=batch_size * 2,
                                          imgsz=imgsz_test,
                                          conf_thres=0.001,
                                          iou_thres=0.7,
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # FQY - Visualization of training data
    ...
    if rank in [-1, 0]:
        ...
        logger.info(f"Final Model Parameters: {num_params}")
        logger.info("=" * 50)
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = test_fuse.test(opt.data,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='./models/yolov5l_fusion_light.yaml', help='model.yaml path')
    ...
    parser.add_argument('--device', default='0', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')# device
    ...
    parser.add_argument('--project', default='runs/f_usion', help='save to project/name')
