...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, train_out = model(img_rgb, augment=augment)  # inference and training outputs
            # out, train_out = model(img_rgb, img_ir, augment=augment)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='./runs/origin_train/exp4/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='./data/multispectral/rgb.yaml', help='*.data path')
    ...
    parser.add_argument('--project', default='runs/origin_test', help='save to project/name')
    ...
    if opt.task in ('train', 'val', 'test'):  # run normally
        ...
        e_t = time.time()
        total_time = e_t - s_t  # Calculate total time
        print(f"Total test time: {total_time:.2f} seconds")  # Print total time
